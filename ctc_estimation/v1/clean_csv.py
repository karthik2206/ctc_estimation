...
def clean_csv(input_file, output_file):
    ...
    if 'current_ctc' in df.columns:
        ...
        # Remove top and bottom 1% outliers from overall data
        initial_rows = len(df)
        lower_bound = df['current_ctc'].quantile(0.01)
        upper_bound = df['current_ctc'].quantile(0.99)
        df = df[(df['current_ctc'] >= lower_bound) & (df['current_ctc'] <= upper_bound)]
        removed_rows = initial_rows - len(df)
        print(f"Removed {removed_rows} rows as overall outliers (top and bottom 1%)")
    ...
    # First apply CTC filtering based on years of experience
    df = filter_ctc_by_experience(df)
    
    # Then remove outliers within each category
